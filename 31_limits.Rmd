...
```{r, #fig-llnsim, fig.cap="As the number of coin tosses goes to infinity, the average probabiity of heads converges to 0.5", echo= FALSE}
set.seed(02138)
n <- 1e3
...
```{r, #fig-seqabc, fig.cap = "Behavior of Some Sequences", echo = FALSE}
seq <- 1:20
...
ggplot(range0, aes(x = x)) +
  ...
  labs(y = expression(f(x)), title = expression(f(x)==frac(x^2+2*x, x))) +
